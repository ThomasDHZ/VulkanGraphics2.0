...
SceneRender2DPipeline::SceneRender2DPipeline() : GraphicsPipeline()
{
}

SceneRender2DPipeline::SceneRender2DPipeline(VulkanEngine& renderer, const VkRenderPass& renderPass, int PipelineBitFlags) : GraphicsPipeline(renderer)
{
    CreateDescriptorSetLayout(renderer);
    CreateShaderPipeLine(renderer, renderPass, PipelineBitFlags);
}

SceneRender2DPipeline::~SceneRender2DPipeline()
{
}

void SceneRender2DPipeline::CreateDescriptorSetLayout(VulkanEngine& renderer)
{
    ...
    LayoutBindingInfo.emplace_back(DescriptorSetLayoutBindingInfo{ 6, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT });
    LayoutBindingInfo.emplace_back(DescriptorSetLayoutBindingInfo{ 7, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT });
    LayoutBindingInfo.emplace_back(DescriptorSetLayoutBindingInfo{ 8, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_FRAGMENT_BIT });
    GraphicsPipeline::CreateDescriptorSetLayout(renderer, LayoutBindingInfo);
}

void SceneRender2DPipeline::CreateShaderPipeLine(VulkanEngine& renderer, const VkRenderPass& renderPass, int PipelineBitFlags)
{
    std::vector<char> vertShaderCode;
    std::vector<char> fragShaderCode;

        vertShaderCode = ReadShaderFile("C:/Users/dotha/source/repos/VulkanGraphics/VulkanGraphics/shaders/Shader2D_BloomOutputVert.spv");
        fragShaderCode = ReadShaderFile("C:/Users/dotha/source/repos/VulkanGraphics/VulkanGraphics/shaders/Shader2D_BloomOutputFrag.spv");
...
void SceneRender2DPipeline::UpdateGraphicsPipeLine(VulkanEngine& renderer, const VkRenderPass& renderPass, int PipelineBitFlags)
{
