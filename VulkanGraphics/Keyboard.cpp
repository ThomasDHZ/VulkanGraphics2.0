...
void Keyboard::Update(std::shared_ptr<VulkanWindow> window, std::shared_ptr<Camera> camera)
{
	...
	else if(auto OCamera = dynamic_cast<OrthographicCamera*>(camera.get()))
	{
		...
		if (glfwGetKey(window->GetWindowPtr(), GLFW_KEY_E) == GLFW_PRESS)
		{
			OCamera->ProcessKeyboard(FORWARD, deltaTime);
		}
		if (glfwGetKey(window->GetWindowPtr(), GLFW_KEY_Q) == GLFW_PRESS)
		{
