...
ReflectionCubeMapRenderPass::ReflectionCubeMapRenderPass(std::shared_ptr<VulkanEngine> engine, std::shared_ptr<RenderedDepthTexture> ShadowMapTexture) : BaseRenderPass()
{
...
void ReflectionCubeMapRenderPass::RebuildSwapChain(std::shared_ptr<RenderedDepthTexture> ShadowMapTexture)
{
    RenderPassResolution = glm::ivec2(EnginePtr::GetEnginePtr()->SwapChain.GetSwapChainResolution().width, EnginePtr::GetEnginePtr()->SwapChain.GetSwapChainResolution().height);
