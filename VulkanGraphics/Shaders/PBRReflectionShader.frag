...
layout(binding = 6) uniform sampler3D Texture3DMap[];
layout(binding = 7) uniform samplerCube CubeMap[];
layout(binding = 8) buffer SphereAreaLightBuffer { SphereAreaLight sphereLight; } sphereLightBuffer[];
layout(binding = 9) buffer TubeAreaLightBuffer { TubeAreaLight tubeAreaLight; } tubeLightBuffer[];
...
layout(binding = 16) uniform sampler2D SpotShadowMap[];
...
{		
      ...
      texture(TextureMap[material.DiffuseMapID], UV).a == 0.0f)
   {
    ...
    vec2 brdf  = texture(TextureMap[34], vec2(max(dot(N, V), 0.0), roughness)).rg;
