...
Font::Font(const std::string FontLocation)
{
	uint32_t MaxWidth = 0;
	...
	else
	{
		...
		for (unsigned char c = 0; c < 128; c++)
		{
			...
			if (c != 32)
			{
				...
				Character character =
				{
					TextureManagerPtr::GetTextureManagerPtr()->LoadTexture2D(glm::ivec2(face->glyph->bitmap.width, face->glyph->bitmap.rows), PixelList, VK_FORMAT_R8G8B8A8_SRGB),
