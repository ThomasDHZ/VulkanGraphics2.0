...
SceneRenderPass::SceneRenderPass(VulkanEngine& engine)
{
    ...
    sceneRenderingPipeline = std::make_shared<SceneRenderingPipeline>(engine, RenderPass, RenderDrawFlags::RenderNormally);
...
void SceneRenderPass::UpdateSwapChain(VulkanEngine& engine)
{
    ...
    sceneRenderingPipeline->UpdateGraphicsPipeLine(engine, RenderPass, RenderDrawFlags::RenderNormally);
