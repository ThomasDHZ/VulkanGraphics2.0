...
void RenderPBRFrameBufferTexturePipeline::SetUpDescriptorSets(std::shared_ptr<RenderedCubeMapTexture> irradianceMap, std::shared_ptr<RenderedCubeMapTexture> prefilterMap, std::shared_ptr<Texture> brdfLUT)
{
    ...
    DescriptorList.emplace_back(EnginePtr::GetEnginePtr()->AddTextureDescriptorSet(10, DescriptorSets, irradiancTextureBufferInfo));
    DescriptorList.emplace_back(EnginePtr::GetEnginePtr()->AddTextureDescriptorSet(11, DescriptorSets, prefilterTextureBufferInfo));
    DescriptorList.emplace_back(EnginePtr::GetEnginePtr()->AddTextureDescriptorSet(12, DescriptorSets, brdfLUTTextureBufferInfo));
...
void RenderPBRFrameBufferTexturePipeline::SetUpShaderPipeLine(const VkRenderPass& renderPass)
{
    ...
    PipelineShaderStageList.emplace_back(EnginePtr::GetEnginePtr()->CreateShader("Shader/PBRTextureRendererShaderVert.spv", VK_SHADER_STAGE_VERTEX_BIT));
    PipelineShaderStageList.emplace_back(EnginePtr::GetEnginePtr()->CreateShader("Shader/PBRTextureRendererShaderFrag.spv", VK_SHADER_STAGE_FRAGMENT_BIT));
