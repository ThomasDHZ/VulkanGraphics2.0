...
DepthDebugPipeline::DepthDebugPipeline(std::shared_ptr<VulkanEngine> engine, std::shared_ptr<AssetManager> assetManager, const VkRenderPass& renderPass, std::shared_ptr<Texture> DepthTexture) : GraphicsPipeline()
{
    SetUpDescriptorPool(engine, assetManager);
    SetUpDescriptorLayout(engine, assetManager);
    SetUpShaderPipeLine(engine, renderPass);
    SetUpDescriptorSets(engine, assetManager, DepthTexture);
}
...
void DepthDebugPipeline::SetUpDescriptorPool(std::shared_ptr<VulkanEngine> engine, std::shared_ptr<AssetManager> assetManager)
{
...
void DepthDebugPipeline::SetUpDescriptorLayout(std::shared_ptr<VulkanEngine> engine, std::shared_ptr<AssetManager> assetManager)
{
...
void DepthDebugPipeline::SetUpDescriptorSets(std::shared_ptr<VulkanEngine> engine, std::shared_ptr<AssetManager> assetManager, std::shared_ptr<Texture> DepthTexture)
{
    DescriptorSets = engine->CreateDescriptorSets(DescriptorPool, DescriptorSetLayout);

    VkDescriptorImageInfo RenderedTextureBufferInfo = engine->AddTextureDescriptor(DepthTexture->View, DepthTexture->Sampler);
...
void DepthDebugPipeline::SetUpShaderPipeLine(std::shared_ptr<VulkanEngine> engine, const VkRenderPass& renderPass)
{
    std::vector<VkPipelineShaderStageCreateInfo> PipelineShaderStageList;
    PipelineShaderStageList.emplace_back(engine->CreateShader("Shader/DepthDebugShaderVert.spv", VK_SHADER_STAGE_VERTEX_BIT));
    PipelineShaderStageList.emplace_back(engine->CreateShader("Shader/DepthDebugShaderFrag.spv", VK_SHADER_STAGE_FRAGMENT_BIT));
    ...
    pushConstantRange.size = sizeof(ConstDepthDebugProperites);
...
void DepthDebugPipeline::UpdateGraphicsPipeLine(std::shared_ptr<VulkanEngine> engine, std::shared_ptr<AssetManager> assetManager, const VkRenderPass& renderPass, std::shared_ptr<Texture> DepthTexture)
{
